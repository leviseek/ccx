...
// TypeScript（ADR-001 双语言：脚本/服务 TS）；erasable syntax（Node 原生跑）
...
// —— 数据契约（ccx.chrono/1 可 diff JSON） ——
export interface Rect { x: number; y: number; w: number; h: number }
export interface SpawnPoint { x: number; y: number; w?: number; h?: number }
export interface SwitchDef extends Rect { id: string; mode: 'latch' | 'window'; holdTicks: number; target: string }
export interface DoorDef extends Rect { id: string; openHint?: boolean }
export interface EchoDef { id: string; uses: number }
export interface CollectibleDef extends Rect { id: string }
export interface LevelDef {
  schema: string; name: string; width: number; height: number;
  spawn: SpawnPoint; finish: Rect;
  solids: Rect[]; doors: DoorDef[]; switches: SwitchDef[];
  echoes: EchoDef[]; collectibles: CollectibleDef[];
  hint?: string;
}

export interface TickInput { left?: boolean; right?: boolean; jump?: boolean }
export interface ActionInput {
  recordStart?: string; recordStop?: string; summon?: string; swap?: string;
}

export interface Snapshot { x: number; y: number; vx: number; vy: number; onGround: boolean }
export interface Entity extends Snapshot { w: number; h: number }
export interface EchoState extends Entity { id: string; active: boolean; t: number; done: boolean }
export interface DoorState { id: string; open: boolean; rect: DoorDef }
export interface GameEvent {
  type: string;
  slot?: string; ticks?: number; x?: number; y?: number; id?: string;
  door?: string; mode?: string; heldTicks?: number; usesLeft?: number; reason?: string;
}
export interface GameState {
  ...
  switchHold: Record<string, number>;
  won: boolean;
  log: GameEvent[];
  recActive: boolean[];
}
...
interface Slot {
  id: string; uses: number; blueprint: { length: number; samples: Snapshot[] } | null;
  recActive: boolean; samples: Snapshot[]; echo: { t: number } | null;
}
...
export function validateLevel(data: LevelDef): { ok: boolean; errors: string[] } {
  const errs: string[] = [];
...
export function createGame(level: LevelDef): Game {
  ...
  game.reset = () => {
    ...
    game.state = {
      ...
      echoes: game.slots.map((s) => ({ id: s.id, active: false, t: 0,
        x: 0, y: 0, w: L.spawn.w ?? 0.6, h: L.spawn.h ?? 0.9, vx: 0, vy: 0, onGround: false, done: false })),
  ...
  game.tick = (input: TickInput = {}) => {
    ...
    const bodies: { id: string; rect: Rect }[] = [
      { id: 'player', rect: p },
      ...game.slots.map((_, i) => ({ id: st.echoes[i].id, rect: st.echoes[i] as Rect }))
        .filter((_, i) => st.echoes[i].active),
    ];
...
function snapshot(ent: Entity): Snapshot {
  return { x: ent.x, y: ent.y, vx: ent.vx, vy: ent.vy, onGround: ent.onGround };
}

function _playerPhysics(p: Entity, solidRects: Rect[], solids: Rect[], input: TickInput) {
...
export function actions(game: Game, input: ActionInput): GameEvent[] {
  const ev: GameEvent[] = [];
  const st = game.state;
  game.slots.forEach((slot, i) => {
