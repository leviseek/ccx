// 生成器：napi 绑定源码 / .d.ts / JSON Schema / quickjs C 绑定（W5a QuickJS 主选）
import { analyzeType } from './idl.mjs';

// QuickJS C 绑定目标（script-engine-decision：QuickJS 主选；宿主接口由引擎侧实现）
// 输出：模块注册 + 每方法 JSValue 包装（marshal number/string/bool；数组/回调 v0.1 标 TODO）
export function generateQuickjs(ir) {
  const mod = ir.module.replace(/\./g, '_');
  const L = [];
  L.push('// Auto-generated by ccx-bindgen (quickjs target). DO NOT EDIT.');
  L.push('// 依赖宿主：实现 extern 函数（签名见各方法），并提供 JS_ToFloat64 等标准 quickjs.h API');
  L.push('#include <stdint.h>');
  L.push('#include <string.h>');
  L.push('');
  for (const cls of ir.classes) {
    const snaked = cls.name.replace(/([a-z])([A-Z])/g, '$1_$2').toLowerCase();
    for (const m of cls.methods) {
      const ret = m.ret === 'string' ? 'const char*' : m.ret === 'void' ? 'void' : m.ret;
      // C 绑定类型（QuickJS C API）：string -> const char*（区别于 C++ 的 std::string）
      const cType = (t) => t === 'string' ? 'const char*' : (t === 'bool' ? 'bool' : cppType(t));
      const plist = m.params.map((p) => cType(p.type) + ' ' + p.name).join(', ');
      L.push('extern ' + ret + ' ' + m.name + '(' + plist + ');');
      L.push('static JSValue ccx_' + mod + '_' + m.name +
             '(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {');
      L.push('  (void)this_val;');
      // 参数 marshal
      if (m.ret === 'string') {
        L.push('  const char* result = (const char*)0;');
      } else if (m.ret === 'void') {
        L.push('  /* void */');
      } else {
        L.push('  ' + m.ret + ' result = 0;');
      }
      for (let i = 0; i < m.params.length; ++i) {
        const p = m.params[i];
        if (p.type === 'string') {
          L.push('  const char* a' + i + ' = argc > ' + i +
                 ' && JS_IsString(argv[' + i + ']) ? JS_ToCString(ctx, argv[' + i + ']) : "";');
        } else if (p.type === 'bool') {
          L.push('  bool a' + i + ' = argc > ' + i + ' ? JS_ToBool(ctx, argv[' + i + ']) : false;');
        } else {
          L.push('  double a' + i + ' = 0;');
          L.push('  if (argc > ' + i + ') JS_ToFloat64(ctx, &a' + i + ', argv[' + i + ']);');
        }
      }
      const args = m.params.map((p, i) => (p.type === 'string') ? 'a' + i
                    : (p.type === 'bool') ? 'a' + i : '(float)a' + i).join(', ');
      if (m.ret === 'string') {
        L.push('  result = ' + m.name + '(' + args + ');');
        L.push('  if (!result) return JS_NewString(ctx, "");');
        L.push('  JSValue out = JS_NewString(ctx, result);');
        L.push('  return out;');
      } else if (m.ret === 'void') {
        L.push('  ' + m.name + '(' + args + ');');
        L.push('  return JS_UNDEFINED;');
      } else {
        L.push('  result = (float)' + m.name + '(' + args + ');');
        L.push('  (void)result;');
        L.push('  return JS_NewFloat64(ctx, (double)' + m.name + '(' + args + '));');
      }
      L.push('}');
      L.push('');
    }
  }
  L.push('static const JSCFunctionListEntry ccx_' + mod + '_methods[] = {');
  for (const cls of ir.classes) {
    for (const m of cls.methods) {
      L.push('  JS_CFUNC_DEF("' + m.name + '", ' + m.params.length + ', ccx_' + mod + '_' + m.name + '),');
    }
  }
  L.push('};');
  L.push('static int ccx_' + mod + '_module_init(JSContext* ctx, JSModuleDef* m) {');
  L.push('  return JS_SetModuleExportList(ctx, m, ccx_' + mod + '_methods,');
  L.push('                                sizeof(ccx_' + mod + '_methods) / sizeof(JSCFunctionListEntry));');
  L.push('}');
  L.push('// 宿主按模块注册：JS_NewCModule(ctx, "' + ir.module + '", ccx_' + mod + '_module_init)');
  L.push('');
  return L.join('\n');
}
